...
export const api={
  ...
  publish:(batch:ImportBatch)=>request<ObjectResult<ImportBatch>>(`/imports/${batch.id}:publish`,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({version:batch.version||1,reviewNote:'已完成样本核对'})}),
